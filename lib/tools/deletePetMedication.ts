import { z } from 'zod'
import { getMedication, deleteMedication } from '@/lib/pets'
import type { MCPToolContext } from './types'

export const deletePetMedicationSchema = z.object({
  medicationId: z.string().describe('ID of the medication to delete (e.g. med-101)'),
})

export type DeletePetMedicationInput = z.infer<typeof deletePetMedicationSchema>
...
async function initiateCIBA(
  ...
  console.log('[deletePetMedication] initiating CIBA — sub:', ctx.sub, '| message:', bindingMessage)
  ...
  if (!res.ok) {
    ...
    console.error('[deletePetMedication] CIBA initiation failed — status:', res.status, '| body:', err)
  ...
  console.log('[deletePetMedication] CIBA initiated — auth_req_id:', data.auth_req_id, '| interval:', data.interval ?? 5)
...
async function pollForApproval(authReqId: string, intervalSeconds: number): Promise<void> {
  ...
  while (Date.now() < deadline) {
    ...
    if (res.ok) {
      console.log('[deletePetMedication] CIBA approved')
      return
    }

    const { error } = data
    if (error === 'authorization_pending') {
      console.log('[deletePetMedication] CIBA pending — next poll in', pollMs / 1000, 's')
      continue
    } else if (error === 'slow_down') {
      pollMs += 5_000
      console.log('[deletePetMedication] CIBA slow_down — new interval:', pollMs / 1000, 's')
      continue
...
/**
 * Executes the deletePetMedication MCP tool.
 *
 * Authorization:
 ...
 *   2. Core — deletes the medication once approved.
 */
export async function executeDeletePetMedication(
  params: DeletePetMedicationInput,
  ctx: MCPToolContext,
) {
  const med = getMedication(params.medicationId)
  if (!med) throw new Error(`Medication "${params.medicationId}" not found.`)

  console.log('[deletePetMedication] called with:', {
  ...
  const { authReqId, interval } = await initiateCIBA(
    ctx,
    `Approve deletion of medication ${med.name} (${params.medicationId})`,
  )
  await pollForApproval(authReqId, interval)

  const deleted = deleteMedication(params.medicationId)
  if (!deleted) throw new Error(`Medication "${params.medicationId}" not found after approval.`)

  return {
    auth_pattern: 'CIBA (Client-Initiated Backchannel Authentication) — push notification sent to user device; deletion executed after approval',
    success: true,
    deleted,
  }
}
